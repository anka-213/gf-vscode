...
export class ExtensionLogger implements Logger {
	...
	private logLevel(level: LogLevel, msg: string) {
		if (level <= this.level) {
			this.write(
				`[${this.name}] ${LogLevel[level].toUpperCase()} ${msg}`
			);
		}
	}
...
export async function httpsGetSilently(
					...
					getWithRedirects(opts, (res) => {
						...
						const needsGunzip =
							res.headers['content-type'] ===
								'application/gzip' ||
							extname(srcUrl.path ?? '') === '.gz';
						const needsUnzip =
							res.headers['content-type'] === 'application/zip' ||
							extname(srcUrl.path ?? '') === '.zip';
						if (needsGunzip) {
						...
						res.on('data', (chunk: Buffer) => {
							...
							const msg = `${toMB(curSize).toFixed(1)}MB / ${toMB(
								totalSize
							).toFixed(1)}MB`;
							progress.report({
								message: msg,
...
function getWithRedirects(
	...
	f: (res: http.IncomingMessage) => void
): http.ClientRequest {
...
export function executableExists(exe: string): boolean {
	...
	return (
		out.status === 0 || (which.sync(exe, { nothrow: true }) ?? '') !== ''
	);
}
